namespace octet {
	...
	//{
	//	if(collisionPair.m_pProxy0->m_collisionFilterGroup == 4 && collisionPair.m_pProxy1->m_collisionFilterGroup == 8 || collisionPair.m_pProxy0->m_collisionFilterGroup == 8 && collisionPair.m_pProxy1->m_collisionFilterGroup == 4)
	//	{
	//		printf("near\n");
	//	}
  ...
  public:
    ...
    btRigidBody* get_body(unsigned int i)
    {
	    if(i < rigid_bodies.size())
		    return rigid_bodies[i];
	    return NULL;
    }

	void set_angular_damping(int handle, btScalar s){
		btRigidBody *rigid_body = rigid_bodies[handle];
		btScalar a = rigid_body->getAngularDamping();
	}
	...
	void remove_constraint(btTypedConstraint *c)
	{
		world.removeConstraint(c);
	}

	void add_constraint(btTypedConstraint *c)
	{
    ...
    int add_rigid_body(const mat4t &modelToWorld, const vec4 &param, bool is_dynamic, body_kind kind) {
      ...
      rigid_body->setCollisionFlags((int)!is_dynamic);
      rigid_body->setUserPointer(&world);
