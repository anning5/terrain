...
namespace octet {
  class bump_shader : public shader {
    ...
    GLuint modelToLight_index;     // second matrix used for lighting maps model to camera space
    GLuint draw_shadow_map_index;
    GLuint draw_shadow_index;
    GLuint screen_size_index;
    ...
    //GLuint shadow_map_sampler_index;
    ...
    void init_uniforms(const char *vertex_shader, const char *fragment_shader) {
      ...
      modelToLight_index = glGetUniformLocation(program(), "modelToLight");
      draw_shadow_map_index = glGetUniformLocation(program(), "draw_shadow_map");
      draw_shadow_index = glGetUniformLocation(program(), "draw_shadow");
      screen_size_index = glGetUniformLocation(program(), "screen_size");
      ...
      //shadow_map_sampler_index = glGetUniformLocation(program(), "shadow_map_sampler");
    }
  ...
  public:
	...
	varying float depth;
        ...
        void main() {
          ...
          gl_Position = cameraToProjection * modelToCamera * pos;
	  if(draw_shadow_map == 1)
	  {
		  depth =  gl_Position.z / gl_Position.w;
	  }
	  else if(draw_shadow == 1)
	  {
		  pos_ = cameraToProjection * modelToLight * pos;
	  }
        }
      );
	...
	if(draw_shadow_map == 1)
	{
		...
		gl_FragColor = texture2D(samplers[0], uv_);
		//*/
	...
	else
	{
		...
		if(draw_shadow == 1)
		{
			...
			vec2 lerps;
			lerps.x = fract(screen_size.x * v.x);
			lerps.y = fract(screen_size.y * v.y);
			sourcevals[0] = (texture2D(samplers[6], v.xy ).w + EPSILON < v.z)? 0.3f: 1.0f;  
			sourcevals[1] = (texture2D(samplers[6], v.xy + vec2(1.0/screen_size.x, 0) ).w + EPSILON < v.z)? 0.3f: 1.0f;  
			sourcevals[2] = (texture2D(samplers[6], v.xy + vec2(0, 1.0/screen_size.y) ).w + EPSILON < v.z)? 0.3f: 1.0f;  
			sourcevals[3] = (texture2D(samplers[6], v.xy + vec2(1.0/screen_size.x, 1.0/screen_size.y) ).w + EPSILON < v.z)? 0.3f: 1.0f;  

			///*
			float LightAmount = lerp( lerp( sourcevals[0], sourcevals[1], lerps.x ),
				lerp( sourcevals[2], sourcevals[3], lerps.x ),
				lerps.y );	
			if(texture2D(samplers[6], v.xy).w + EPSILON < v.z)
			{
			gl_FragColor.xyz = .3 * gl_FragColor.xyz;
			}
			gl_FragColor.xyz = LightAmount * gl_FragColor.xyz;
			//*/
		}
    ...
    void render(const mat4t &cameraToProjection, const mat4t &modelToCamera, const vec4 *light_uniforms, int num_light_uniforms, int num_lights, int draw_shadow_map = 0, int draw_shadow = 0, const mat4t &modelToLight = mat4t()) {
	    ...
	    glUniformMatrix4fv(modelToLight_index, 1, GL_FALSE, modelToLight.get());
	    glUniform1i(draw_shadow_map_index, draw_shadow_map);
	    glUniform1i(draw_shadow_index, draw_shadow);
	    glUniform2f(screen_size_index, 752, 730);
	    ...
	    glUniform1iv(samplers_index, 7, samplers);
	    //glUniform1i(shadow_map_sampler_index, 6);
    }
